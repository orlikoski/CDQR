...
def create_reports(dst_loc, csv_file):
	...
	# # Create visability directory and file names
	# viz_dir_name = dst_loc+"\\Visability"
	# viz_evt_name = rpt_dir_name+"\\Event Log Report.time"
	# viz_fs_name = rpt_dir_name+"\\File System Report.time"
	# viz_ih_name = rpt_dir_name+"\\Internet History Report.time"
	# viz_pf_name = rpt_dir_name+"\\Prefetch Report.time"
	# viz_reg_name = rpt_dir_name+"\\Registry Report.time"
	# viz_st_name = rpt_dir_name+"\\Scheduled Tasks Report.time"
	# viz_per_name = rpt_dir_name+"\\Persistence Report.time"
	# viz_si_name = rpt_dir_name+"\\System Information Report.time"
	...
	if not os.path.isfile(csv_file):
		...
		mylogfile.write("File not found " + csv_file+"\n")
		sys.exit(1)
...
mylogfile.write("Processing started at: "+str(start_dt)+"\n")
...
mylogfile.write("Parsing duration was: "+str(duration01)+"\n")
...
mylogfile.write("\nCreating the SuperTimeline CSV file"+"\n")
print("\""+"\" \"".join(command2)+"\"")
mylogfile.write("\""+"\" \"".join(command2)+"\""+"\n")
create_SuperTimeline(command2)
print("SuperTimeline CSV file is created")
mylogfile.write("SuperTimeline CSV file is created\n")
...
mylogfile.write("All reporting complete\n")
# Closing log file
end_dt = datetime.datetime.now()
...
mylogfile.write("Processing/reporting ended at: "+str(end_dt)+"\n")
mylogfile.write("Reporting duration was: "+str(duration02)+"\n")
mylogfile.write("Total duration was: "+str(duration01+duration02)+"\n")
timingfile.close()
mylogfile.close()
