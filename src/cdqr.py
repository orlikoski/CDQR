...
#   7 Reports for Mac and Lin: File System, Internet History, System Information, AntiVirus, Firewall, Mac, and Linux
#  Improved the way existing log files and results directories are handled
#  Ability to create an export file
#
# Known Bugs
# The Plaso 1.4 MFT parser is not functioning (Plaso Error #556) for disk images but filestat and usnjrnl are working fine.  This will be corrected once Plaso 1.4 is updated.
#
# DEPENDANCIES: 
# 1.) Plaso v1.4 static binaries from:
#		Option 1: Plaso for 64-bit MS Visual C++ disto 2010 https://e366e647f8637dd31e0a13f75e5469341a9ab0ee.googledrive.com/host/0B30H7z4S52FleW5vUHBnblJfcjg/1.4.0/plaso-1.4.0-win-amd64-vs2010.zip
#		Option 2: Plaso for 32-bit MS Visual C++ disto 2008 https://e366e647f8637dd31e0a13f75e5469341a9ab0ee.googledrive.com/host/0B30H7z4S52FleW5vUHBnblJfcjg/1.4.0/plaso-1.4.0-win32-vs2008.zip
...
def create_reports(dst_loc, csv_file):
	...
	rpt_st_search = re.compile(r'winjob,|windows_task_cache,')
	...
	rpt_si_search = re.compile(r'rplog,|explorer_mountpoints2,|explorer_programscache,|windows_shutdown,|windows_timezone,|windows_usb_devices,|windows_usbstor_devices,|windows_version,')
	...
	rpt_mac_search = re.compile(r'mac_keychain,|mac_securityd,|mactime,|plist,|airport,|apple_id,|ipod_device,|macosx_bluetooth,|macosx_install_history,|macuser,|maxos_software_update,|plist_default,|spotlight,|spotlight_volume,|time_machine,|appusage,|mackeeper_cache,')
	rpt_lin_search = re.compile(r'bsm_log,|popularity_contest,|selinux,|utmp,|utmpx,')
	# Create a list of the report names
	if parser_opt == "datt":
		lor = [rpt_evt_name,rpt_fsfs_name,rpt_fsmft_name,rpt_fsusnjrnl_name,rpt_ih_name,rpt_pf_name,rpt_reg_name,rpt_st_name,rpt_per_name,rpt_si_name,rpt_av_name,rpt_fw_name,rpt_mac_name,rpt_lin_name]
	elif parser_opt == "win":
		lor = [rpt_evt_name,rpt_fsfs_name,rpt_fsmft_name,rpt_fsusnjrnl_name,rpt_ih_name,rpt_pf_name,rpt_reg_name,rpt_st_name,rpt_per_name,rpt_si_name,rpt_av_name,rpt_fw_name]
	else:
		lor = [rpt_fsfs_name,rpt_ih_name,rpt_si_name,rpt_av_name,rpt_fw_name,rpt_mac_name,rpt_lin_name]
	...
	if create_rep:
		...
		if parser_opt == "datt":
			...
			rpt_mac = open(rpt_mac_name,'a+', encoding='utf-8')
			rpt_lin = open(rpt_lin_name,'a+', encoding='utf-8')
			lofh = [[rpt_fsfs_search,rpt_fsfs,rpt_fsfs_name],[rpt_ih_search,rpt_ih,rpt_ih_name],[rpt_si_search,rpt_si,rpt_si_name],[rpt_av_search,rpt_av,rpt_av_name],[rpt_fw_search,rpt_fw,rpt_fw_name],[rpt_mac_search,rpt_mac,rpt_mac_name],[rpt_lin_search,rpt_lin,rpt_lin_name]]
